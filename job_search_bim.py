...
class JobSearchGUI:
    ...
    def create_config_tab(self):
        ...
        # Salarios
        salary_frame = ttk.LabelFrame(config_frame, text="Preferencias Salariales (USD)", padding=10)
        salary_frame.pack(fill='x', padx=10, pady=5)
        
        ttk.Label(salary_frame, text="Salario mínimo:").grid(row=0, column=0, sticky='w')
        self.min_salary = tk.StringVar(value=str(SALARY_PREFERENCES.get('minimum_usd', 30000)))
        ttk.Entry(salary_frame, textvariable=self.min_salary, width=15).grid(row=0, column=1, padx=5)
        
        ttk.Label(salary_frame, text="Salario preferido:").grid(row=1, column=0, sticky='w')
        self.pref_salary = tk.StringVar(value=str(SALARY_PREFERENCES.get('preferred_usd', 60000)))
        ttk.Entry(salary_frame, textvariable=self.pref_salary, width=15).grid(row=1, column=1, padx=5)
        
        # Filtros
        filters_frame = ttk.LabelFrame(config_frame, text="Filtros", padding=10)
        filters_frame.pack(fill='x', padx=10, pady=5)
        
        self.remote_only = tk.BooleanVar(value=SEARCH_FILTERS.get('remote_only', False))
        ttk.Checkbutton(filters_frame, text="Solo empleos remotos", variable=self.remote_only).pack(anchor='w')
        
        self.exclude_agencies = tk.BooleanVar(value=SEARCH_FILTERS.get('exclude_agencies', True))
        ttk.Checkbutton(filters_frame, text="Excluir agencias", variable=self.exclude_agencies).pack(anchor='w')
        
        # Botones
        ttk.Button(config_frame, text="💾 Guardar", command=self.save_config).pack(pady=10)
